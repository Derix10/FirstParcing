...
class PlaylistAdapter(val myContext: Context, private val onClick: (item:Item) -> Unit ) : RecyclerView.Adapter<PlaylistAdapter.PlaylistHolder>() {
   ...
   inner class PlaylistHolder(private var binding: PlaylistItemBinding): RecyclerView.ViewHolder(binding.root) {
        fun onBind(playList: Item) {
            ...
            binding.descriptionItem.text = myContext.getString(R.string.series_count, playList.contentDetails?.itemCount.toString())
